'''
Usage:
blender -b -P scan_single_random.py <model_dir> <output_dir> <model_id> <save_rgbd_image> <save_pc_per_view> <save_pc_complete> <pc_per_view_size> <pc_complete_size>

Example:
Scan the provided model.obj file using 6 cameras randomly located around the object, output rgbd image, partial point cloud (2048 points) from each camera view,
...
blender -b -P scan_single_random.py ./example_obj/model.obj ./output plane 1 1 1 2048 16384
'''
...
def update_random_camera(camera, focus_point, distance):
	'''
	Focus the camera to focus point and place the camera at a specific distance.
	The camera location is sampled uniformly on the unit sphere.
	'''
	np.random.seed()
...
if __name__ == '__main__':
	...
	if os.path.isfile(model_dir):
		...
		for i in range(NUM_FRAMES):
			update_random_camera(camera, focus_point=Vector(FOCUS_POINT), distance=CAMERA_DISTANCE)
